...
// Make sure the pid text doesn't extend onto the status bar.
       ...
       int i = 0;
       for (i = 0; i < strlen(vis->m_pname);i++){
            if (vis->m_pname[i] == ':') {
                vis->m_pname[i] = '\0';
                break;
            }
       }
       char* ptype = vis->m_pname;
       char* pname = &(vis->m_pname[i+1]);

       int offset = strlen(pname)/2 * 4;
       M_WriteText(max(vis->x1 - offset,0), lowest_dc_yl, pname);

       offset = strlen(ptype)/2 * 4;
       M_WriteText(max(vis->x1 - offset,0), max(lowest_dc_yl - 6,0), ptype);

       vis->m_pname[i] = ':';
       // int offset = strlen(vis->m_pname)/2 * 4;
       // M_WriteText(max(vis->x1 - offset,0), lowest_dc_yl, vis->m_pname);
...
// Set the pid and name in the vissprite.
    ...
    strcpy(vis->m_pname, thing->m_pname);
